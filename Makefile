...
config: ## Generate configuration
	docker run -v $(OVPN_DATA):/etc/openvpn --log-driver=none --rm $(NAME) ovpn_genconfig -u udp://$(SERVERNAME)
pki: ## Init PKI
	docker run -v $(OVPN_DATA):/etc/openvpn --log-driver=none --rm -it $(NAME) ovpn_initpki
start: ## Start VPN Server
	docker run -v $(OVPN_DATA):/etc/openvpn -d -p 1194:1194/udp --cap-add=NET_ADMIN $(NAME)
client: ## Create ovpn client file
	docker run -v $(OVPN_DATA):/etc/openvpn --log-driver=none --rm -it $(NAME) easyrsa build-client-full CLIENTNAME nopass
retrieve: ## Retrieve ovpn client file
	docker run -v $(OVPN_DATA):/etc/openvpn --log-driver=none --rm $(NAME) ovpn_getclient CLIENTNAME > CLIENTNAME.ovpn